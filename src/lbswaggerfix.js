...
fs.readFile('../doc/core-swagger-raw.json', (err, data) => {
  if (!err) {
    const jsonswagger = JSON.parse(data.toString());
    asyncLib.eachOf(jsonswagger.paths, (route, path, next) => {
      ...
      fs.writeFile('../doc/core-swagger-fix.json', JSON.stringify(jsonswagger), (err) => {
        //
      });
    });
  }
